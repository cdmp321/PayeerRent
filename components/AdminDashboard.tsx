...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user }) => {
  ...
  // Only Admin sees 'payments' (Methods) & 'users' management full access
  // Manager can see Users but maybe restricted actions (handled in handlers)
  // Both see Deposits, Items, Finances
  
  const displayNavItems = navItems.filter(item => {
      if (user?.role === UserRole.MANAGER) {
          // Manager sees everything except Payment Methods (usually admin only, but let's keep it visible if needed or remove)
          // Requirements say Manager can't see Settings tab in Nav, but we moved it to top right icon.
          // Let's keep Payment Methods for now as per previous behavior, unless specified.
          return true; 
      }
      return true;
  });
  ...
  return (
    <div className="relative">
      {/* Floating Settings Button for Manager (Top Right of Content Area) */}
      {user?.role === UserRole.MANAGER && (
          <button 
             onClick={() => setActiveTab('settings')}
             className="absolute -top-16 right-0 p-2 text-slate-400 hover:text-indigo-600 transition-colors z-40 bg-white/50 rounded-full backdrop-blur-sm shadow-sm border border-white/50"
      ...
      {/* Navigation */}
      <div className="flex flex-wrap gap-2 mb-8 bg-white/50 p-1.5 rounded-2xl backdrop-blur-sm shadow-sm border border-white/60">
        {displayNavItems.map(item => (
          <button
            key={item.id}
            onClick={() => setActiveTab(item.id as any)}
            className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all relative ${
              activeTab === item.id 
          ...
          >
            ...
            {item.count ? (
              <span className={`absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full text-[10px] text-white ring-2 ring-white ${item.id === 'deposits' ? 'bg-orange-500' : 'bg-emerald-500'}`}>
                {item.count}
              </span>
            ) : null}
          </button>
      ...
      <div className="flex justify-end mb-4">
          <button 
             ...
             className="flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-indigo-600 transition-colors bg-white px-3 py-1.5 rounded-lg shadow-sm border border-gray-100"
          >
